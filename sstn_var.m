function [STFT,SST1,SST2,SST3,SST4,omega,omega2,omega3,omega4,tau2,tau3,phi22p,phi33p,phi44p] = sstn_var(s,gamma,sigma,ft,bt)
...
% Initialization
STFT = zeros(neta,nb);
SST1 = zeros(neta,nb, 3);
SST2 = zeros(neta,nb, 3);
SST3 = zeros(neta,nb, 3);
SST4 = zeros(neta,nb, 3);
omega = zeros(neta,nb);
tau2 = zeros(neta,nb);
tau3 = zeros(neta,nb);
tau4 = zeros(neta,nb);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%SST1%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
            k = 1 + round(omega(eta,b) - 1/2);          
            if k>=1 && k<=neta               
             SST1(k,b, 1)  = SST1(k,b,1) + STFT(eta,b);
            end         
            
            k = 1+round(omega(eta,b));          
            if k>=1 && k<=neta               
             SST1(k,b, 2)  = SST1(k,b,2) + STFT(eta,b);
            end
            
            k = 1+round(omega(eta,b) + 1/2);          
            if k>=1 && k<=neta               
             SST1(k,b, 3)  = SST1(k,b,3) + STFT(eta,b);
            end
            
%%%%%%%%%%%%%%%%%%%%SST2%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%          
            k = 1 + round(omega2(eta,b) - 1/2);          
            if k>=1 && k<=neta               
             SST2(k,b, 1)  = SST2(k,b,1) + STFT(eta,b);
            end         
            
            k = 1+round(omega2(eta,b));          
            if k>=1 && k<=neta               
             SST2(k,b, 2)  = SST2(k,b,2) + STFT(eta,b);
            end
            
            k = 1+round(omega2(eta,b) + 1/2);          
            if k>=1 && k<=neta               
             SST2(k,b, 3)  = SST2(k,b,3) + STFT(eta,b);
            end
             
%%%%%%%%%%%%%%%%%%%%%SST3%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%             
            k = 1 + round(omega3(eta,b) - 1/2);          
            if k>=1 && k<=neta               
             SST3(k,b, 1)  = SST3(k,b,1) + STFT(eta,b);
            end         
            
            k = 1+round(omega3(eta,b));          
            if k>=1 && k<=neta               
             SST3(k,b, 2)  = SST3(k,b,2) + STFT(eta,b);
            end
            
            k = 1+round(omega3(eta,b) + 1/2);          
            if k>=1 && k<=neta               
             SST3(k,b, 3)  = SST3(k,b,3) + STFT(eta,b);
            end
             
%%%%%%%%%%%%%%%%%%%%%SST4%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            k = 1 + round(omega4(eta,b) - 1/2);          
            if k>=1 && k<=neta               
             SST4(k,b, 1)  = SST4(k,b,1) + STFT(eta,b);
            end         
            
            k = 1+round(omega4(eta,b));          
            if k>=1 && k<=neta               
             SST4(k,b, 2)  = SST4(k,b,2) + STFT(eta,b);
            end
            
            k = 1+round(omega4(eta,b) + 1/2);          
            if k>=1 && k<=neta               
             SST4(k,b, 3)  = SST4(k,b,3) + STFT(eta,b);
            end
